...
LISTITEM configbox_s[20] =
{
   ...
   { "Double click function",    &config.dclick_command,  funcclick_proc,   funcdraw_proc, "The function to execute when the left mouse button is double-clicked" },
   { "Auto fold files",          &config.auto_fold,       strclick_proc,    strdraw_proc,  "Files with these extensions will automatically be folded" },
   { "Auto indent files",        &config.auto_indent,     strclick_proc,    strdraw_proc,  "Files with these extensions will have automatic indentation turned on" },
   { "Binary files",             &config.bin,             strclick_proc,    strdraw_proc,  "Files with these extensions will be loaded in binary mode" },
   { "Hex mode files",           &config.hex,             strclick_proc,    strdraw_proc,  "Files with these extensions will be loaded in hex editing mode" },
   { "Browse files",             &config.makefiles,       strclick_proc,    strdraw_proc,  "Files with these extensions will be loaded by the browse command" },
   { "Wordwrap files",           &config.wrap,            strclick_proc,    strdraw_proc,  "Files with these extensions will have wordwrap turned on" },
   { "Wrap at column",           &config.wrap_col,        numclick_proc,    numdraw_proc,  "The default column at which to wordwrap" },
   { "Use big cursor",           &config.big_cursor,      boolclick_proc,   booldraw_proc, "If set, a tall editing cursor is displayed" },
   { "Make backups",             &config.make_backups,    boolclick_proc,   booldraw_proc, "If set, when saving files the old versions are renamed to *.bak" },
   { "Check for changed files",  &config.check_files,     boolclick_proc,   booldraw_proc, "If set, checks for files being changed by external programs" },
   { "Strip spaces on save",     &config.save_strip,      boolclick_proc,   booldraw_proc, "If set, trailing spaces are removed and tabs are inserted when saving files" },
   { "Print only ascii chars",   &config.print_ascii,     boolclick_proc,   booldraw_proc, "If set, extended ascii characters are replaced with spaces during printing" },
   { "External tab size",        &config.load_tabs,       numclick_proc,    numdraw_proc,  "The tab size used by external files, for the read/write routines" },
   { "Screensaver delay (mins)", &config.screen_save,     numclick_proc,    numdraw_proc,  "The delay before the screensaver kicks in. Zero for no screensaver" },
   { "Undo levels",              &config.undo_levels,     numclick_proc,    numdraw_proc,  "The number of stored undo operations (per file)" },
   { "File search",              &config.file_search,     numclick_proc,    numdraw_proc,  "How many directories to recurse when looking for a file" }
};
