...
void paint(HDC dc)
{
	...
	for (y = 0; y < screen_h; y++) {
		...
		while (x < screen_w) {
			...
			fgc = known_colors[screen[y][x].col & 15];
			bgc = known_colors[screen[y][x].col >> 4];
...
void window_thread(HANDLE hInstance)
{
	...
	wndclass.hIcon = LoadIconA(hInstance, ("wnd_icon"));
	...
	BOOL b = ShowWindow(hwnd, (maximize) ? SW_SHOWMAXIMIZED : SW_SHOW);
	b = UpdateWindow(hwnd);
...
int PASCAL WinMain(HANDLE hInstance, HANDLE hPrevInstance, LPSTR lpszCmdParam, int nCmdShow)
{
	...
	char mp[1024];
	GetModuleFileNameA(GetModuleHandleA(NULL), mp, sizeof(mp));
	if (argc == 0) {
		argv[0] = mp;
	}
